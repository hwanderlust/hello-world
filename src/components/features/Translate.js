...
class Translate extends React.Component {
      ...
      .then(data => {
        ...
        if (data["data"]["detections"][0][0]["language"]) {
          ...
          if (langOption) {
            this.setState(
              {
                detectedLang: { name: langOption.name, code: langOption.code }
              },
              () => console.log(this.state)
            );
          }
  ...
  render() {
    const renderLanguages = () => {
      return googleLanguages.map(lang => (
        <option key={lang.code} id={lang.code}>
          {lang.name}
        </option>
      ));
    };
      ...
      <React.Fragment>
        ...
        <div className="translate-form">
          ...
          <select
            ...
            name="fromLang"
            value={this.state.detectedLang.name}
          >
            {renderLanguages()}
          </select>
        </div>
        ...
        <div className="translate-form">
          ...
          <select onChange={this.handleChange} name="toLang">
            <option key="default" id="default" disabled selected>
              Choose one
            </option>
            {renderLanguages()}
          </select>
