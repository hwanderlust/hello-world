...
class ChatContainer extends React.Component {
  ...
  handleNewChat = user => {
    getUser(user.recipient_id)
      ...
      .then(data => {
        createChat({ ...user, sender_id: this.props.currentUser.id }).then(
          chat => {
            this.props.updateChat(chat.id);
            const chatObj = {
              ...chat,
              recipient_user: {
                id: this.props.recipientUser.id,
                username: this.props.recipientUser.username
              },
              chatbox: { x: user.x, y: user.y, color: user.color }
            };
  ...
  render() {
    const renderChatComponents = () => {
      if (this.state.users) {
        return (
          <Chat users={this.state.users} handleNewChat={this.handleNewChat} />
        );
      }
    };

    return <div className="chat-container">{renderChatComponents()}</div>;
  }
}
